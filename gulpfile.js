...
gulp.task("copyFonts", function () {
	return gulp
		.src(dirs.source + "/assets/fonts/**/*.{woff,woff2,ttf,otf,eot,svg}")
...
gulp.task("copyCSS", function () {
	return gulp
		.src(dirs.source + "/assets/css/**/*.css")
...
gulp.task("php", function () {
	return gulp
		.src(dirs.source + "/**/**/**/*.php") // какие файлы обрабатывать (путь из константы, маска имени)
		.pipe(plumber({ errorHandler: onError }))
		.pipe(replace(/\n\s*<!--DEV[\s\S]+?-->/gm, "")) // убираем комментарии <!--DEV ... -->
		.pipe(gulp.dest(dirs.build)); // записываем файлы (путь из константы)
});
